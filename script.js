...
document.addEventListener("DOMContentLoaded", () => {
  const viewButtons = document.querySelectorAll(".toggle-btn .view-btn");

  viewButtons.forEach(button => {
    button.addEventListener("click", e => {
      e.stopPropagation(); // Prevent triggering parent clicks

      const familyBlock = button.closest(".family-collapsible");
      const content = familyBlock.querySelector(".collapsible-content");
    ...
    document.querySelectorAll('.btn-approve, .btn-reject').forEach(btn => {
      btn.addEventListener('click', async e => {
        ...
        const file = parent.dataset.file;
        ...
        if (response.ok) {
          showFlash(`${file} ${action}d successfully.`, action === 'approve' ? 'success' : 'warning');
          parent.remove();
        } else {
          showFlash('Error performing action.', 'error');
        }
      });
    });
